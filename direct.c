...
{
    DL_SORT(ibl->vektor, blackMoveCompare);
    return;
}

static void sortWhiteMoves(BOARDLIST* ibl)
{
    DL_SORT(ibl->vektor, whiteMoveCompare);
    return;
}
...
void solve_direct(DIR_SOL* dsol, BOARD* startpos)
{
    ...
    if ((opt_actual == true) && (opt_moves == 1)) {
        ...
        DL_COUNT(dsol->keys->vektor, bd, ct);
        ...
        if (shortsol == false) {
            ...
            DL_COUNT(dsol->keys->vektor, b, ct);
...
static BOARDLIST* gloss_whiteMidMove(BOARD* inBrd, int move, int lastmove)
{
    ...
    {
        if (opt_aim == MATE) {
            DL_FOREACH_SAFE(wml->vektor, m, tmp) {
                if (shortStipAchieved == true) {
                    DL_DELETE(wml->vektor, m);
                    freeBoard(m);
                } else {
                    if (m->check == true) {
...
static BOARDLIST* norm_final_move(BOARD* inBrd, int move)
{
    ...
    if (opt_aim == MATE) {
        DL_FOREACH_SAFE(wml->vektor, bd, tmp) {
            if (bd->check == true) {
        ...
        DL_FOREACH_SAFE(bbl->vektor, bm, tmp) {
            if (bm->mover != KING) {
                ...
                if (wbl != NULL) {
                    ...
                    DL_COUNT(threats->vektor, tmp1, ct);

                    if (ct == c) {
                        DL_DELETE(bbl->vektor, bm);
                        freeBoard(bm);
                    }
                }
            }
...
static void sortTriesKeys(DIR_SOL* ds)
{
    ...
    DL_FOREACH(ds->trieskeys->vektor, b) {
        bc = cloneBoard(b);

        if (isKey(b) == true) {
            DL_APPEND(g_keys->vektor, bc);
        } else {
            DL_APPEND(g_tries->vektor, bc);
        }
    }
    ds->tries = g_tries;
    ds->keys = g_keys;
...
static void deTrivialise(BOARDLIST* wml)
{
    ...
    int ct;
    assert(wml != NULL);
    DL_FOREACH_SAFE(wml->vektor, wm, tmp) {
        bml = wm->nextply;
...
static BOARDLIST* calculateSetPlay(BOARD* inBrd)
{
    ...
    DL_FOREACH_SAFE(bList->vektor, ourBrd, tmp) {
        if (opt_moves > 2) {
